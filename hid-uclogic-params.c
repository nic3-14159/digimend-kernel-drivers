	...
	struct uclogic_params *p)
{
	static const u8 init_packet[] = {
		0x02, 0xb0, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00
	};
	...
	p->pen.desc_ptr = uclogic_rdesc_template_apply(
		uclogic_rdesc_xppen_star06_pen_arr,
		uclogic_rdesc_xppen_star06_pen_size,
		desc_params,
		ARRAY_SIZE(desc_params)
	);
	p->pen.desc_size = uclogic_rdesc_xppen_star06_pen_size;
	p->pen.id = 0x02;

	rc = uclogic_params_frame_init_with_desc(
		&p->frame_list[0],
		uclogic_rdesc_xppen_star06_frame_arr,
		uclogic_rdesc_xppen_star06_frame_size,
		UCLOGIC_RDESC_V1_FRAME_ID
	);
	if (rc < 0) {
...
#define VID_PID(_vid, _pid) \
	...
	case VID_PID(USB_VENDOR_ID_UGEE,
		...
		if (rc != 0) {
			hid_err(hdev, "Star 06 init failed: %d\n", rc);
			goto cleanup;
		}
